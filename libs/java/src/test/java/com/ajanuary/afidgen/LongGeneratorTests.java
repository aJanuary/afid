...
public class LongGeneratorTests {
  ...
  @Test
  void exposes_prefix() {
    var generator = AFID.longGenerator("ent");
  ...
  void errors_if_random_is_null() {
    var thrown = assertThrows(NullPointerException.class, () -> AFID.longGenerator("ent", null));
    assertThat(thrown).hasMessageThat().isEqualTo("random");
  }

  @Test
  void stream_generates_ids() {
    var ids = AFID.longGenerator("ent").stream().limit(3).toList();

    assertAll(
        () -> assertThat(ids).hasSize(3),
        () -> assertThat(ids.get(0)).matches("^ent-" + B32_CLASS + "{5}-" + B32_CLASS + "{20}$"),
        () -> assertThat(ids.get(1)).matches("^ent-" + B32_CLASS + "{5}-" + B32_CLASS + "{20}$"),
        () -> assertThat(ids.get(2)).matches("^ent-" + B32_CLASS + "{5}-" + B32_CLASS + "{20}$"));
  }
}
